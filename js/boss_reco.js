document.addEventListener('DOMContentLoaded', function () {
    ...
    // 筛选收起
    const screenMenu = document.querySelector('.screen_menu');
    const screenTag = document.querySelector('.screen_tag');
    const main = document.querySelector('.main');
    const right = document.querySelector('.right');
    const jobInfo = document.querySelector('.job_info');
    const jobMain = document.querySelector('.job_main');


    screenMenu.addEventListener('click', function () {
        const isCollapsed = screenMenu.classList.toggle('collapsed');
        screenTag.classList.toggle('collapsed');

        if (isCollapsed) {
            screenTag.style.maxHeight = '0';
            screenTag.style.padding = '0';
            main.style.marginTop = '110px';
            right.style.top = '110px';
            right.style.height = '670px';
            jobInfo.style.height = '415px';
            jobMain.style.height = '490px';


        } else {
            screenTag.style.maxHeight = '56px';
            screenTag.style.padding = '0';
            main.style.marginTop = '160px';
            right.style.top = '160px';
            right.style.height = '610px';
            jobInfo.style.height = '355px';
            jobMain.style.height = '430px';
        }
    });
    ...
    clearFiltersLink.addEventListener('click', function (event) {
        ...
        [tagEx1Options, tagEx2Options, tagEx3Options, tagEx4Options].forEach(options => {
        ...
        [tagEx1, tagEx2, tagEx3, tagEx4].forEach(tagEx => {
    ...
    toggleButton.addEventListener('click', function () {
        if (changeFlag) {
            toggleButton.textContent = '切换为推荐简历';
            toggleButton.style.backgroundImage = 'linear-gradient(315deg, #00B09B 0%, #96C93D 74%)';
        } else {
            toggleButton.textContent = '切换为已收简历';
            toggleButton.style.backgroundImage = 'linear-gradient(315deg, #6A82FB 0%, #FC5C7D 74%)';
        }
    ...
    talks.forEach(function (e) {
        e.addEventListener('click', function () {
            window.location.href = 'boss_chat.html';
        });
    });
